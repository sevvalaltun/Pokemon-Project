...
void initializePlayer(Pokemon Pokemons[], Player *player1, Player *player2) {
    int used[1015] = {0}; // Player1 ve Player2 için ortak

    // Player1 için 6 Pokémon seç
    for(int i = 0; i < 6; i++) {
        int r;
        do {
            r = rand() % 1015;
        } while(used[r]); // daha önce seçilen Pokémonu tekrar seçme
    ...
    // Player1 Pokémonlarını rastgele karıştır
    for(int i = 5; i > 0; i--) {
        int j = rand() % (i+1);
        Pokemon temp = player1->pokemons[i];
        player1->pokemons[i] = player1->pokemons[j];
        player1->pokemons[j] = temp;
    }

    // Player2 için 6 Pokémon seç
    for(int i = 0; i < 6; i++) {
        int r;
        do {
            r = rand() % 1015;
        } while(used[r]); // Player1 ile aynı Pokémonu seçme
    ...
    // Player2 Pokémonlarını rastgele karıştır
    for(int i = 5; i > 0; i--) {
        int j = rand() % (i+1);
        Pokemon temp = player2->pokemons[i];
        player2->pokemons[i] = player2->pokemons[j];
        player2->pokemons[j] = temp;
    }
}
